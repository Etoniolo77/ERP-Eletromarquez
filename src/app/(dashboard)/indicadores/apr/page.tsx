...
export default function AprDigitalPage() {
    ...
    const loadData = async (forceSync = false) => {
        ...
        try {
            ...
            const resp = await api.get(`/apr/dashboard?sector=${sector}&periodo=${period}`)
            setData(resp.data)
        } catch (err: any) {
    ...
    const lineChartData = data.history.labels.map((label, idx) => ({
        name: label,
        value: data.history.values[idx]
    }))
        ...
        <div className="p-4 space-y-6 animate-in fade-in duration-700">
            ...
            <div className="grid gap-4 grid-cols-1 md:grid-cols-4">
                <KpiCard
                    ...
                    value={`${data.stats.aderencia_global.toFixed(1)}%`}
                    variation={data.stats.aderencia_var}
                    target={`${data.meta}% Meta`}
                    icon="shield"
                    colorValue={data.stats.aderencia_global >= data.meta ? "success" : "danger"}
                />
                <KpiCard
                    title="Equipes Analisadas"
                    value={`${data.stats.total_equipes}`}
                    variation={data.stats.total_equipes_var}
                    target="No Período"
                    icon="groups"
                    colorValue="primary"
                />
                <KpiCard
                    ...
                    value={`${data.stats.equipes_fora_meta}`}
                    variation={data.stats.equipes_fora_var}
