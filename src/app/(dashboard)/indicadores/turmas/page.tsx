...
interface DashboardData {
    ...
    top_piores: { equipe: string, csd: string, produtividade: number, ociosidade: number }[]
    top_melhores: { equipe: string, csd: string, produtividade: number, notas: number }[]
    breakdown_csd: {
        name: string
        num_equipes: number
        acima_meta: number
...
export default function TurmasPage() {
    ...
    const loadData = async (forceSync = false) => {
        ...
        try {
            ...
            const response = await api.get(`/produtividade/dashboard?periodo=${period}&view=${view}`)
    ...
    if (!mounted || (loading && !dashData)) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
                ...
                <p className="text-sm font-semibold text-slate-500 animate-pulse uppercase tracking-widest">Compilando Equipes de Turmas...</p>
            </div>
        )
    }
                    ...
                    <div className="flex-1 overflow-auto custom-scrollbar">
                        <table className="w-full text-left">
                            <thead className="bg-surface/50 text-[10px] font-semibold uppercase text-text-muted">
                                <tr>
                                    <th className="p-3 pl-4 min-w-[300px]">Equipe</th>
                                    <th className="p-3 text-center">Exec</th>
                                    <th className="p-3 text-center">Rej</th>
                                    <th className="p-3 text-center">Inter</th>
                                    <th className="p-3 text-center">Ocio</th>
                                    <th className="p-3 text-right pr-4">Prod</th>
                                </tr>
                            </thead>
