...
export default function TurmasRDOPage() {
    ...
    const loadData = async (forceRefresh = false) => {
        ...
        try {
            ...
            const response = await api.get(`/turmas_rdo/dashboard?periodo=${period}&regional=${regional}`)
    ...
    if (loading && !dashData) return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
            ...
            <p className="text-sm font-semibold text-text-muted animate-pulse uppercase tracking-widest">Compilando Painel de Turmas...</p>
        </div>
    )
    if (error) return (
        <div className="p-8 border border-rose-500/20 bg-rose-500/5 text-rose-500 rounded-sm m-4 flex flex-col items-center justify-center gap-4">
            ...
            <p className="font-semibold text-xs uppercase tracking-widest text-center">Falha no rastreio operacional: {error}</p>
        </div>
    )
                ...
                <div className="flex-1 bg-surface border border-border p-4 min-h-[105px] rounded-sm flex items-stretch gap-6 shadow-sm">
                    ...
                    <div className="flex-1 flex flex-col justify-between py-1">
                        <div className="flex items-center gap-2">
                            ...
                            <h3 className="text-[10px] font-semibold uppercase text-text-muted tracking-[0.2em]">Inteligência de RDO e Insights</h3>
                        </div>
                            ...
                            {dashData.insights?.slice(0, 2).map((ins: any, i: number) => (
                                <p key={i} className="text-[11px] font-medium text-text-heading leading-tight border-l-2 border-primary/30 pl-3">
                                    ...
                                    <p className="text-[11px] font-medium text-text-heading leading-tight border-l-2 border-primary/30 pl-3">
                                        Monitoramento de conformidade de RDO Turmas consolidado para {dashData.period_label}.
                                    </p>
                                )}
                        </div>
                    </div>
            ...
            <div className="bg-surface border border-border rounded-sm overflow-hidden flex flex-col">
                <div className="p-4 border-b border-border bg-surface/50 flex items-center justify-between">
                    ...
                    >
                        ...
                        <option value="BSF">Barra de S. Francisco</option>
